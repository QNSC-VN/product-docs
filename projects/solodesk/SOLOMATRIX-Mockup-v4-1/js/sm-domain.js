...
(function (global) {
  ...
  /* ─── Ngày lịch — dùng chung cho đồng hồ 30 ngày [Q-001] và Trạm (PLAN C.3) ─── */

  /** Cộng n ngày LỊCH (T7, CN tính như thường) → chuỗi iso YYYY-MM-DD. */
  ...
  /** Số ngày nguyên từ a tới b (b − a, có thể âm = đã quá hạn). */
  function ngayGiua(a, b) {
    return Math.round((new Date(b) - new Date(a)) / 86400000);
  }

  /** Đồng hồ 30 ngày đăng ký hoá đơn điện tử [Q-001] — một hàm chung cho toàn app (C.3).
      luc = t.vuotLuc: NGÀY CUỐI KỲ TÍNH THUẾ có doanh thu lũy kế vượt 1 tỷ (seed ghi sẵn),
      KHÔNG phải ngày giao dịch vượt. Chưa vượt → null (Trạm hiện «canh ngưỡng»). */
  function mocVuotNguong(t, ky) {
    ky = ky || 'quy';                 // giả định kỳ quý — chờ Q-023
    const luc = t.vuotLuc;
    if (!luc) return null;
    const han = congNgay(luc, 30);    // 30 ngày LỊCH [Q-001]
    return { ky, luc, han, conLai: ngayGiua(SM.CLOCK.today, han),
             nhan: ky === 'quy' ? 'giả định kỳ quý — chờ Q-023' : '' };
  }
  ...
  function taxEstimate(t, period) {
    ...
    // Thuế sàn đã NỘP THAY hộ [Q-019 — NĐ 117/2025 Đ11 k4]: kênh sàn khấu trừ rồi nộp,
    // app chỉ đối chiếu chứ không khai lại. Đơn kênh sàn = channel shopee/tiktok/lazada (qua chuyenId).
    const rr = periodRange(period);
    const sanDaNopThay = (t.orders || []).reduce((s, o) =>
      (o.date >= rr.from && o.date <= rr.to && (o.state === 'paid' || o.state === 'done') &&
        SAN_IDS.indexOf(veIdMoi(o.channel)) >= 0) ? s + (o.thueSanDaNop || 0) : s, 0);
    const byGroup = {};
    let revenue = 0;
    lines.forEach(l => {
      revenue += l.amount;
    ...
    if (isDN) {
      ...
      return { regime: 'dn', period: periodRange(period), revenue, byGroup: Object.values(byGroup),
               ...
               total: gtgt + tndn, mienThue: false, sanDaNopThay };
    }
    ...
    return { regime: 'hkd', period: periodRange(period), revenue, yearRev, mienThue,
             byGroup: Object.values(byGroup), gtgt, tncn, tndn: 0, total: gtgt + tncn, sanDaNopThay };
  }
  ...
  function deadlines(t) {
    ...
    // v4: đồng hồ 30 ngày đăng ký hoá đơn điện tử [Q-001] — chỉ khi hộ ĐÃ vượt ngưỡng 1 tỷ (C.3).
    // Đặt SAU khối gán DICH để giữ dichDen 'thue' (thẻ nguong-mtt ở trên vẫn trỏ về tab Bán).
    const moc30 = mocVuotNguong(t);
    if (moc30) list.push({
      id: 'nguong-30n', loai: 'nguong', dichDen: 'thue',
      ten: moc30.conLai >= 0
        ? `Còn ${moc30.conLai} ngày đăng ký hoá đơn điện tử — hết ngày ${SM.fmt.dmy(moc30.han)}`
        : `Đã quá hạn ${-moc30.conLai} ngày — cần cán bộ`,
      han: moc30.han, conLai: moc30.conLai, soTien: null,
      trangThai: moc30.conLai < 0 || moc30.conLai <= 14 ? 'todo' : 'warn',
    });

    // v4 F2 [R2 #19 · chốt P1]: lượt xuất hoá đơn bị HOÃN chờ chữ ký số phải thấy được
    // mỗi ngày — chặn đúng nghiệp vụ nhưng KHÔNG chặn câm: bán vẫn ghi, cán bộ theo nối.
    if (t.choCTS > 0) {
      const ctsNoi = (connectors(t).find(c => c.id === 'cts') || {}).noi;
      list.push({
        id: 'cho-cts', loai: 'chocts', dichDen: ctsNoi ? 'ban' : 'ketnoi',
        ten: ctsNoi
          ? `${t.choCTS} hoá đơn còn chờ xuất lại — chữ ký số đã nối xong, vào việc là phát hành được`
          : `${t.choCTS} lần xuất hoá đơn đang chờ nối chữ ký số — bán và thu tiền vẫn ghi bình thường`,
        han: null, conLai: null, trangThai: ctsNoi ? 'warn' : 'todo',
      });
    }

    // v4: kênh không gửi dữ liệu — nghi ngờ kết nối đứt (D-#5, bảng C.15).
    const dut = connectors(t).filter(c => c.trangThai === 'chet');
    if (dut.length) {
      const c0 = dut[0];
      const nNgay = c0.lanDongBoCuoi ? ngayGiua(String(c0.lanDongBoCuoi).slice(0, 10), SM.CLOCK.today) : null;
      list.push({
        id: 'ketnoi-dut', loai: 'ketnoi', dichDen: 'ketnoi',
        ten: `Kênh ${c0.ten} không gửi dữ liệu${nNgay !== null ? ' ' + nNgay + ' ngày' : ''} — có thể kết nối đứt` +
             (dut.length > 1 ? ` (và ${dut.length - 1} kênh khác)` : ''),
        han: null, conLai: null, trangThai: 'todo', refs: dut.map(c => c.id),
      });
    }
  ...
  const CHANNELS = {
    ...
    'goi-dien': { ten: 'Khách gọi điện đặt trước', icon: '📞' },   // F2 R2 #01 — đơn tay
  ...
  function addPurchase(t, p) {
    // F2 R2 #06: CHẶN bảng kê trùng người bán + cùng hàng + cùng ngày — thiếu giấy tờ
    // thì phải MỞ bảng kê cũ mà bổ sung (D.suaBangKe); lập mới là đếm chi phí hai lần.
    if (!p.boQuaTrung) {
      const trung = (t.purchases || []).find(x =>
        String(x.seller || '').trim().toLowerCase() === String(p.seller || '').trim().toLowerCase() &&
        x.sku === p.sku && x.date === (p.date || SM.CLOCK.today));
      if (trung) return {
        ok: false, id: trung.id, trung: trung.id,
        lyDo: 'Bà con ' + trung.seller + ' đã có bảng kê ' + trung.id + ' cho hàng này trong ngày ' +
              SM.fmt.dmy(trung.date) + ' — mở bảng kê đó bổ sung giấy tờ thay vì lập mới, lập mới là đếm chi phí hai lần',
      };
    }
  ...
  function addBooking(t, req) {
    ...
    // F2 R2 #05: cọc là tiền thật — kiểm tra hợp lệ TRƯỚC khi ghi lượt đặt.
    if (req.coc && req.coc.soTien > 0 && req.total && req.coc.soTien > req.total)
      return { ok: false, lyDo: 'Tiền cọc vượt tổng tiền lượt đặt' };
    ...
    t.bookings.push(rec);
    // F2 R2 #05: nhận cọc lúc đặt phải vào dòng tiền NGAY (qua D.nhanCoc → D.thuTien),
    // không để tối kết ca lệch đúng số cọc. Cùng một đường với màn «Ghi cọc».
    if (req.coc && req.coc.soTien > 0) SM.dom.nhanCoc(t, rec.id, req.coc);
    SM.save();
    return { ok: true, rec };
  }
  ...
  function qrPoints(t, date) {
    ...
    // F2 R2 #05: cọc nhận trong ngày mà đường cũ ghi thẳng b.coc (chưa qua dòng tiền)
    // vẫn là THU THẬT — cộng vào «Thu hôm nay» để tối đối tiền không lệch số cọc.
    // Cọc đã ghi dòng tiền (có paymentId) hoặc đã xử lý xong (daXuLyTien) thì không cộng nữa;
    // lượt đặt huỷ rồi mà cọc chưa trả khách thì tiền vẫn còn — máy không giả sử đã hoàn.
    const macDinh = pts.length ? pts[0].id : 'q1';
    (t.bookings || []).forEach(b => {
      if (!b.coc || b.coc.ngay !== d || b.coc.paymentId || b.coc.daXuLyTien) return;
      const p = pts.find(x => x.id === macDinh);
      if (p) { p.revenue += b.coc.soTien; p.count += 1; }
    });
  ...
  /**
   * Chốt P1 [R2 #19]: chưa nối chữ ký số thì KHÔNG phát hành hoá đơn điện tử —
   * nhưng KHÔNG chặn câm: trả về LỜI NÓI THƯỜNG để hộ hiểu (bán vẫn ghi, chỉ
   * hoá đơn đang chờ, cán bộ theo giúp nối). Nhóm LUẬT không có «bỏ qua».
   * Trả null nghĩa là được phát hành bình thường.
   */
  function chanPhatHanh(t) {
    const cts = connectors(t).find(c => c.id === 'cts');
    if (cts && cts.noi) return null;
    const cb = ((t.onboarding || {}).coCanBo) || {};
    return {
      ok: false, choHoan: true, viec: 'cho-chu-ky-so',
      lyDo: 'Chưa nối xong chữ ký số nên máy chưa phát hành được hoá đơn điện tử. ' +
            'Việc bán và thu tiền vẫn ghi bình thường, chỉ hoá đơn đang chờ. ' +
            (cb.maCanBo ? 'Cán bộ ' + cb.maCanBo + ' đang theo giúp nối chữ ký số' : 'Cán bộ hỗ trợ địa bàn sẽ theo giúp nối chữ ký số') +
            ' — nối xong là xuất được ngay, cô chú khỏi làm lại.',
    };
  }
   ...
   * Chưa nối chữ ký số thì KHÔNG phát hành (chốt P1) — trả lời thường + đếm lượt
   * hoãn cho thẻ nhắc, bán và thu tiền không vướng.
   */
  function issueInvoice(t, o) {
    const chan = chanPhatHanh(t);
    if (chan) {
      t.choCTS = (t.choCTS || 0) + 1;   // đếm lượt hoãn — hiện ở «Việc cần để ý»
      const dong = { viec: 'Hoãn phát hành hoá đơn — chờ chữ ký số', doiTuong: null,
                     truoc: o.total || null, sau: null, lyDo: 'chưa nối chữ ký số' };
      if (SM.ops && SM.ops.ghiNhatKy) SM.ops.ghiNhatKy(t, dong);
      else {
        t.nhatKy = t.nhatKy || [];
        t.nhatKy.push(Object.assign({ id: 'NK' + String(t.nhatKy.length + 1).padStart(4, '0'),
          luc: new Date().toISOString(), ngay: SM.CLOCK.today, ai: 'Hộ kinh doanh' }, dong));
      }
      SM.save();
      return chan;
    }
  ...
  function programBoard() {
    /* Claude verify 20/08: hộ trắng chưa kích hoạt (cd4-moi — không activatedAt, không license)
       không thuộc bảng thanh toán ba cửa của Chương trình — lọc ra để «Hộ tham gia» đếm đúng. */
    const rows = SM.tenantIds().filter(id => { const t = SM.tenant(id); return t && (t.activatedAt || (t.license && t.license.ma)); }).map(id => {
  ...
  function invoiceForOrg(t, src, buyer) {
    ...
    // F2 R2 #19 (chốt P1): chưa nối chữ ký số → hoãn phát hành. Đơn giữ nguyên,
    // KHÔNG ghi công nợ cho hoá đơn chưa tồn tại — trả lời thường cho hộ đọc.
  ...
  /* ─── Bảng chuyển id v4 ← v3 (PLAN C.0) — ánh xạ DUY NHẤT của cả app, module khác đọc qua đây ─── */
  const chuyenId = {
    bank: ['bank', 'qr'], zalooa: ['zalo'], cts: ['cks'], etax: ['etax', 'cthue'],
    ketoan: ['ntqg'], shipper: ['ghn', 'ghtk', 'vtp'],
  }; // id không có trong map = giữ nguyên (hddt, shopee, tiktok, lazada, pos, booking)

  const CU_SANG_MOI = {};                       // dựng ngược 1 lần: id cũ → id v4
  Object.keys(chuyenId).forEach(moi => chuyenId[moi].forEach(cu => { CU_SANG_MOI[cu] = moi; }));
  const veIdMoi = id => CU_SANG_MOI[id] || id;  // id mới đi qua nguyên vẹn, id cũ dịch về đúng connector

  const SAN_IDS = ['shopee', 'tiktok', 'lazada']; // kênh sàn — thuế sàn nộp thay [Q-019]

  // Độ tươi công bố (bảng C.15) — dòng chưa có bằng chứng đo thì ghi nguyên nhãn, không bỏ trống (R-A3-04).
  const CHUA_DO = q => 'chưa đo — radar đang hỏi ' + q;

  /* 12 connector v4 — cờ «bắt buộc» phẳng của v3 đã BỎ (B.14): nghĩa vụ LUẬT chỉ sinh
     khi hộ vượt 1 tỷ (xem connectorSummary), không còn auto-noi. */
  const CONNECTORS = [
    { id: 'bank',    nhom: 'Tiền', ten: 'Tài khoản ngân hàng + nhận tiền QR',
      moTa: 'Tiền về tới đâu app biết ngay, khỏi mở app ngân hàng đối chiếu [Q-002]',
      doTuoi: { congBo: 'giây–phút', coChe: 'webhook', pollAnToan: 'đối soát số dư sau 24 giờ [Q-003]', nguon: '[Q-006]' } },
    { id: 'zalooa',  nhom: 'Mạng xã hội bán hàng', ten: 'Zalo OA (trang kinh doanh Zalo)',
      moTa: 'Kênh chạm khách chính ở tỉnh — khách nhắn 22 giờ cũng có câu trả lời soạn sẵn',
      doTuoi: { congBo: CHUA_DO('Q-034'), coChe: 'webhook', pollAnToan: null, nguon: 'Q-034' } },
    { id: 'hddt',    nhom: 'Thuế và hoá đơn', ten: 'Hoá đơn điện tử',
      moTa: 'Khách sạn, nhà hàng đòi hoá đơn là xuất được ngay — gồm hoá đơn khởi tạo từ máy tính tiền',
      doTuoi: { congBo: '15 phút–1 ngày làm việc', coChe: null, pollAnToan: 'không áp — bên trả lời sau', nguon: '[Q-002]' } },
    { id: 'cts',     nhom: 'Thuế và hoá đơn', ten: 'Chữ ký số',
      moTa: 'Ký tờ khai và hoá đơn — chữ ký này của cô/chú, không ai khác dùng được',
      doTuoi: { congBo: 'không áp — không có luồng dữ liệu', coChe: null, pollAnToan: null, nguon: '[Q-002]' } },
    { id: 'etax',    nhom: 'Thuế và hoá đơn', ten: 'Kê khai thuế điện tử',
      moTa: 'Tờ khai và nộp thuế một nơi — app điền sẵn, hộ bấm «Nộp»',
      doTuoi: { congBo: 'thủ công', coChe: 'thu-cong', pollAnToan: 'không áp', nguon: '[Q-002]' } },
    { id: 'ketoan',  nhom: 'Nền tảng dùng chung của Nhà nước', ten: 'Sổ kế toán (nền tảng Nhà nước)',
      moTa: 'Theo Điều 10 Nghị định 20/2026 — chưa vận hành', chuaCo: true,
      doTuoi: { congBo: CHUA_DO('Q-009/Q-037'), coChe: null, pollAnToan: null, nguon: 'Q-009 · Q-037' } },
    { id: 'shopee',  nhom: 'Sàn thương mại điện tử', ten: 'Gian hàng Shopee',
      moTa: 'Đơn và tồn kho hai chiều — từ 27/05/2026 chỉ nhận kết nối qua Open Platform [Q-002]',
      doTuoi: { congBo: CHUA_DO('Q-036'), coChe: 'webhook', pollAnToan: 'đối soát đơn sau 24 giờ [Q-003]', nguon: 'Q-036' } },
    { id: 'tiktok',  nhom: 'Sàn thương mại điện tử', ten: 'Gian hàng TikTok Shop',
      moTa: 'Đơn, tồn kho, phát trực tiếp',
      doTuoi: { congBo: CHUA_DO('Q-042'), coChe: 'webhook', pollAnToan: 'đối soát đơn sau 24 giờ [Q-003]', nguon: 'Q-042' } },
    { id: 'lazada',  nhom: 'Sàn thương mại điện tử', ten: 'Gian hàng Lazada',
      moTa: 'Đơn và tồn kho',
      doTuoi: { congBo: CHUA_DO('Q-042'), coChe: 'webhook', pollAnToan: 'đối soát đơn sau 24 giờ [Q-003]', nguon: 'Q-042' } },
    { id: 'shipper', nhom: 'Vận chuyển', ten: 'Đơn vị vận chuyển',
      moTa: 'GHN, GHTK, Viettel Post — chọn hãng bên trong; tiền thu hộ về thẳng tài khoản của hộ',
      doTuoi: { congBo: CHUA_DO('Q-037'), coChe: 'webhook', pollAnToan: 'đối soát vận đơn sau 24 giờ [Q-003]', nguon: 'Q-037' } },
    { id: 'pos',     nhom: 'Bán tại quầy', ten: 'Máy tính tiền',
      moTa: 'Hoá đơn điện tử khởi tạo từ máy tính tiền kết nối dữ liệu cơ quan thuế [Q-001]',
      doTuoi: { congBo: CHUA_DO('Q-037/Q-044'), coChe: 'polling', pollAnToan: 'theo giới hạn tần suất', nguon: 'Q-037 · Q-044' } },
    { id: 'booking', nhom: 'Nền tảng đặt phòng, đặt tour', ten: 'Nền tảng đặt phòng/đặt tour',
      moTa: 'Booking, Agoda, Traveloka — đại diện nhóm nền tảng đặt trước', nganh: 'Dịch vụ du lịch',
      doTuoi: { congBo: CHUA_DO('Q-045'), coChe: 'webhook', pollAnToan: 'đối soát đặt chỗ sau 24 giờ [Q-003]', nguon: 'Q-045' } },
  ];

  /** Danh sách kết nối của một hộ: lọc theo ngành + trạng thái đã nối.
      v4 (B.14): HẾT auto-noi «bắt buộc» — cờ noi chỉ đến từ t.connections (seed/sự kiện)
      hoặc máy trạng thái onboarding B.2 (datTrangThaiKetNoi). Mặc định chưa nối. */
  function connectors(t) {
    ...
    return CONNECTORS
      ...
      .map(c => {
        // migrate (C.0): dòng còn ghi id cũ (qr, cks, ghn…) thì mượn về connector v4 tương ứng
        let st = t.connections[c.id];
        if (!st) {
          const cu = (chuyenId[c.id] || []).find(x => t.connections[x]);
          if (cu) st = t.connections[cu];
        }
        const kn = knAll[c.id] ||
          (chuyenId[c.id] || []).map(x => knAll[x]).filter(Boolean)[0] || null;
        const daNoi = !!(st && st.noi) || !!(kn && kn.trangThai === 'da_ket_noi');
        return Object.assign({}, c, {
          noi: daNoi,
          tuNgay: (st && st.tuNgay) || (kn && kn.batDauLuc) || null,
          trangThai: (st && st.trangThai) ? st.trangThai : (daNoi ? 'ok' : 'chua-noi'), // 'ok' | 'chua-noi' | 'chet' (D-#5)
          lanDongBoCuoi: (st && st.dongBoCuoi) || null, // mốc GIỜ seed ghi — không tự sinh từ CLOCK
          donVe: (t.orders || []).filter(o => veIdMoi(o.channel) === c.id).length,
        });
      });
  }

  /** ĐƯỜNG CHUYỂN TRẠNG THÁI KẾT NỐI DUY NHẤT (máy trạng thái B.2) — thay nút «Nối» giả của v3.
      Kết nối chỉ đến từ: sự kiện ngoài xác nhận (sm-inbox) hoặc cán bộ xác nhận tay —
      lần nào cũng để lại nhật ký. Không còn đường hộ tự bật cờ «đã nối». */
  function datTrangThaiKetNoi(t, ma, trangThai, meta) {
    const HOP_LE = ['chua_hoi', 'chua_co_tk', 'dang_dang_ky', 'cho_duyet', 'da_ket_noi', 'loi', 'bo_qua'];
    const VIEC_MO_TA = {
      chua_hoi: 'Chưa hỏi lại hộ về kênh này', chua_co_tk: 'Hộ chưa có tài khoản của kênh',
      dang_dang_ky: 'Hộ đang làm dở đăng ký kênh', cho_duyet: 'Đăng ký kênh — đang chờ duyệt',
      da_ket_noi: 'Kênh được nối', loi: 'Kênh lỗi — dừng nhận dữ liệu', bo_qua: 'Hộ để kênh này sau',
    };
    ma = veIdMoi(ma); // gọi nhầm id cũ vẫn ghi đúng khoá v4
    if (HOP_LE.indexOf(trangThai) < 0) return { ok: false, lyDo: 'Trạng thái không thuộc máy trạng thái kết nối' };
    t.onboarding = t.onboarding || {};
    t.onboarding.ketNoi = t.onboarding.ketNoi || {};
    const truoc = (t.onboarding.ketNoi[ma] || {}).trangThai || null;
    t.onboarding.ketNoi[ma] = Object.assign({}, t.onboarding.ketNoi[ma],
      { trangThai: trangThai }, meta || {}, { luc: new Date().toISOString() });
    // nhật ký tenant — cùng hình dạng SM.ops.ghiNhatKy (module chưa nạp thì tự ghi)
    const dong = { viec: VIEC_MO_TA[trangThai], doiTuong: ma, truoc, sau: trangThai,
                   lyDo: (meta && (meta.lyDo || meta.aiBam)) || null };
    if (SM.ops && SM.ops.ghiNhatKy) SM.ops.ghiNhatKy(t, dong);
    else {
      t.nhatKy = t.nhatKy || [];
      t.nhatKy.push(Object.assign({ id: 'NK' + String(t.nhatKy.length + 1).padStart(4, '0'),
    ...
    SM.save();
    return { ok: true, ma, trangThai, truoc };
  }

  /** Đếm theo nhóm để đối chiếu danh mục tối thiểu của IV.3 — v4 tách 2 lớp (B.7/B.14):
      «tối thiểu theo CHƯƠNG TRÌNH» = ngưỡng THUMOI giữ nguyên bên dưới;
      «tối thiểu theo LUẬT» = bộ HĐĐT + CTS + kê khai điện tử, CHỈ sinh khi hộ đã vượt 1 tỷ [Q-001]. */
  function connectorSummary(t) {
    ...
    // v4 gộp đại diện (C.0): shipper/booking là MỘT connector cho cả nhóm hãng/nền tảng nên
    // ngưỡng nhóm đó là 1 connector đã nối; «3 sàn» giữ 3 vì còn đúng 3 gian hàng riêng.
    // (Nhóm «App giao đồ ăn» của v3 không còn dòng riêng — food tham chiếu nhóm booking, Q-042/Q-045.)
    const toiThieu = { 'Sàn thương mại điện tử': 3, 'Vận chuyển': 1, 'Nền tảng đặt phòng, đặt tour': 1 };
    const out = Object.values(bag).map(g => {
    ...
    // Lớp LUẬT [Q-001]: đã vượt 1 tỷ lũy kế → phải đủ bộ trong 30 ngày kể từ cuối kỳ vượt
    if (mocVuotNguong(t)) {
      const muc = ['hddt', 'cts', 'etax'];
      const noi = rows.filter(c => muc.indexOf(c.id) >= 0 && c.noi).length;
      out.unshift({
        nhom: 'Theo LUẬT — đã vượt 1 tỷ [Q-001]', tong: muc.length, noi,
        lop: 'luat', muc, toiThieu: muc.length, choVanHanh: false, dat: noi >= muc.length,
      });
    }
    return out;
  }
  ...
  SM.dom = {
    ...
    connectors, connectorSummary, datTrangThaiKetNoi, chuyenId,
    congNgay, ngayGiua, mocVuotNguong,
    ...
    issueInvoice, filingSteps, chanPhatHanh,
...
/* ═══════════════════════════════════════════════════════════════════════════
   ĐỢT F2 — DÒNG TIỀN & ĐƠN TAY (20/08/2026, sửa theo 8 báo cáo review)
   R2 #01 (đơn khách gọi điện) · #02 (thu tiền đúng hình thức) · #03 (khớp tiền
   vào đơn) · #05 (cọc vào dòng tiền) · #06 (bổ sung giấy tờ bảng kê) · R4 V9
   (huỷ đơn phải có lý do). Lõi tiền nằm ở đây, giao diện gọi qua SM.dom.
   ═══════════════════════════════════════════════════════════════════════════ */
(function (global) {
  'use strict';
  const SM = global.SM, D = SM.dom;

  /** Nhật ký thao tác — mượn sổ của sm-ops nếu đã nạp, chưa thì tự ghi cùng hình dạng. */
  function nhatKy(t, m) {
    if (SM.ops && SM.ops.ghiNhatKy) return SM.ops.ghiNhatKy(t, m);
    t.nhatKy = t.nhatKy || [];
    t.nhatKy.push(Object.assign({ id: 'NK' + String(t.nhatKy.length + 1).padStart(4, '0'),
      luc: new Date().toISOString(), ngay: SM.CLOCK.today, ai: 'Hộ kinh doanh' }, m));
    return t.nhatKy[t.nhatKy.length - 1];
  }

  /* ───────── THU TIỀN BẰNG TAY (R2 #02) — đúng hình thức, đúng dòng tiền ──── */

  const HINH_THUC_THU = {
    'tien-mat':     { ten: 'Khách đưa tiền mặt', vaoNganKeo: true },
    'chuyen-khoan': { ten: 'Khách chuyển khoản', vaoNganKeo: false },
    'qr':           { ten: 'Khách quét mã',      vaoNganKeo: false },
    'the':          { ten: 'Khách quẹt thẻ',     vaoNganKeo: false },
  };

  /**
   * Thu tiền bằng tay. Trước đây mọi lần thu đều ghi cứng 'QR' nên tiền mặt
   * thành chuyển khoản — tối kết ca đối tiền lệch MỖI ngày (R2 #02).
   * Trả {ok, phieu}; sai tham số trả {ok:false, lyDo}.
   */
  function thuTien(t, m) {
    if (!(m.soTien > 0)) return { ok: false, lyDo: 'Số tiền phải lớn hơn 0' };
    const ht = HINH_THUC_THU[m.hinhThuc];
    if (!ht) return { ok: false, lyDo: 'Chưa chọn hình thức thu tiền: tiền mặt, chuyển khoản, quét mã hay quẹt thẻ' };
    let don = null;
    if (m.orderId) {
      don = (t.orders || []).find(o => o.id === m.orderId);
      if (!don) return { ok: false, lyDo: 'Không tìm thấy đơn ' + m.orderId };
      if (don.state === 'cancel') return { ok: false, lyDo: 'Đơn ' + don.id + ' đã huỷ rồi — không thu vào đơn này' };
    }
    t.payments = t.payments || [];
    const phieu = {
      id: 'PT-' + SM.CLOCK.today.slice(2, 4) + SM.CLOCK.today.slice(5, 7) + SM.CLOCK.today.slice(8, 10) +
          '-' + String(t.payments.length + 1).padStart(3, '0'),
      date: SM.CLOCK.today,
      point: m.diem || ((t.qrPoints || [])[0] || { id: 'q1' }).id,   // điểm thu — màn quầy nhiều điểm nên truyền đúng diem
      amount: m.soTien,
      method: m.hinhThuc,
      orderId: don ? don.id : null,
      ghiChu: m.ghiChu || null,
      noiDung: m.ghiChu || (don ? 'Thu tiền cho đơn ' + don.id + ' — ' + ht.ten : ht.ten),
      luc: new Date().toISOString(),
    };
    if (m.loai) phieu.loai = m.loai;                 // 'coc' — cọc lượt đặt, không phải tiền bán
    if (m.veDatCho) phieu.veDatCho = m.veDatCho;     // tiền dính lượt đặt — khỏi lọt màn khớp đơn
    t.payments.push(phieu);
    if (don && don.state !== 'paid' && don.state !== 'done') {
      don.state = 'paid'; don.paidAt = SM.CLOCK.today;
    }
    nhatKy(t, { viec: 'Thu tiền — ' + ht.ten, doiTuong: phieu.id, sau: m.soTien,
                lyDo: don ? 'thu cho đơn ' + don.id : (m.ghiChu || null) });
    SM.save();
    return { ok: true, phieu };
  }

  /* ───────── ĐƠN CHO KHÁCH GỌI ĐIỆN / ĐẶT TRƯỚC (R2 #01) ───────── */

  /**
   * Lập đơn bán ra cho khách gọi điện hoặc nhắn đặt trước — việc thường ngày
   * mà trước đây phải ghi giấy nhớ, quên là mất tiền hàng (R2 #01).
   * Dùng lại đường kiểm tồn của checkOrder: hết hàng là nói rõ còn bao nhiêu.
   */
  function taoDonTay(t, m) {
    const khach = String(m.khach || '').trim();
    if (!khach) return { ok: false, lyDo: 'Thiếu tên khách' };
    if (!(m.lines || []).length) return { ok: false, lyDo: 'Chưa có món nào trong đơn' };
    const kenh = ['goi-dien', 'quay', 'zalo'].indexOf(m.kenh) >= 0 ? m.kenh : 'goi-dien';
    const lines = [];
    for (const l of m.lines) {
      if (!(l.qty > 0)) return { ok: false, lyDo: 'Số lượng phải lớn hơn 0' };
      const meta = (t.skus || []).find(s => s.sku === l.sku);
      const gia = (l.price !== undefined && l.price !== null) ? l.price : (meta ? meta.price : null);
      if (!(gia > 0)) return { ok: false, lyDo: 'Không tìm thấy mặt hàng ' + l.sku + ' trong danh mục' };
      lines.push({ sku: l.sku, qty: l.qty, amount: l.qty * gia });
    }
    const chk = D.checkOrder(t, lines);
    if (!chk.ok) return {
      ok: false, viPham: chk.viPham,
      lyDo: 'Không đủ hàng: ' + chk.viPham.map(v => v.name + ' chỉ còn ' + v.con + ' (khách đặt ' + v.xin + ')').join('; '),
    };
    t.orders = t.orders || [];
    const o = {
      id: 'DH-' + SM.CLOCK.today.slice(2, 4) + SM.CLOCK.today.slice(5, 7) + '-' + String(800 + t.orders.length).slice(-3),
      date: SM.CLOCK.today, channel: kenh, buyer: khach, sdt: m.sdt || null,
      state: 'new', lines, total: lines.reduce((s, l) => s + l.amount, 0),
      taxGroup: t.taxGroupDefault, tuTay: true, giaoNgay: m.giaoNgay || null,
      ghiChu: m.ghiChu || null, synced: true,
    };
    t.orders.push(o);
    nhatKy(t, { viec: 'Ghi đơn bán cho khách gọi điện', doiTuong: o.id, sau: o.total,
                lyDo: m.ghiChu || (m.giaoNgay ? 'giao ngày ' + SM.fmt.dmy(m.giaoNgay) : null) });
    SM.save();
    return { ok: true, order: o };
  }

  /* ───────── KHỚP TIỀN VỀ VÀO ĐƠN (R2 #03) ───────── */

  /** Tiền đã về mà CHƯA gắn cho đơn nào — cho màn khớp tay (R2 #03). */
  function tienChuaKhop(t) {
    return (t.payments || []).filter(p => p.choKhop && !p.orderId);
  }

  /** Gán một dòng tiền đã về cho đơn — sổ tiền và danh sách đơn cùng khớp, hết treo «mới» mãi. */
  function khopTienVaoDon(t, paymentId, orderId) {
    const p = (t.payments || []).find(x => x.id === paymentId);
    if (!p) return { ok: false, lyDo: 'Không tìm thấy dòng tiền' };
    const o = (t.orders || []).find(x => x.id === orderId);
    if (!o) return { ok: false, lyDo: 'Không tìm thấy đơn ' + orderId };
    if (p.orderId && p.orderId !== orderId) return { ok: false, lyDo: 'Tiền này đã gắn cho đơn ' + p.orderId + ' rồi' };
    if (o.state === 'cancel') return { ok: false, lyDo: 'Đơn ' + o.id + ' đã huỷ — không gắn tiền vào đơn đã huỷ' };
    p.orderId = o.id;
    delete p.choKhop;
    if (o.state !== 'paid' && o.state !== 'done') { o.state = 'paid'; o.paidAt = SM.CLOCK.today; }
    nhatKy(t, { viec: 'Khớp tiền về vào đơn', doiTuong: o.id, sau: p.amount,
                lyDo: 'tiền ' + SM.fmt.d(p.amount) + ' gắn cho đơn ' + o.id });
    SM.save();
    return { ok: true, phieu: p, order: o };
  }

  /* ───────── HUỶ ĐƠN CÓ LÝ DO (R4 V9) ───────── */

  /** Huỷ đơn — BẮT BUỘC lý do: huỷ là trả hàng về kho, sau này hỏi lại phải giải trình được. */
  function huyDon(t, orderId, lyDo) {
    if (!String(lyDo || '').trim()) return { ok: false, lyDo: 'Phải ghi lý do huỷ — sau này hỏi lại phải trả lời được' };
    const o = (t.orders || []).find(x => x.id === orderId);
    if (!o) return { ok: false, lyDo: 'Không tìm thấy đơn ' + orderId };
    if (o.state === 'cancel') return { ok: false, lyDo: 'Đơn này đã huỷ rồi' };
    if (o.invoiceId) return { ok: false, lyDo: 'Đơn này đã xuất hoá đơn ' + o.invoiceId + ' — xử lý hoá đơn ở mục Hoá đơn trước, rồi mới huỷ đơn' };
    (o.lines || []).forEach(l => {
      const lot = (t.lots || []).find(x => x.sku === l.sku);
      if (lot) lot.qty += l.qty;      // trả hàng về lô — cùng kiểu advanceOrder, giữ đường truy xuất
    });
    o.state = 'cancel';
    o.huyNgay = SM.CLOCK.today; o.huyLyDo = String(lyDo).trim();
    nhatKy(t, { viec: 'Huỷ đơn', doiTuong: o.id, truoc: o.total, sau: 0, lyDo: o.huyLyDo });
    SM.enqueue('channel', 'Báo huỷ đơn ' + o.id + ' về kênh ' + ((D.CHANNELS[o.channel] || {}).ten || o.channel),
      { tenant: t.id, id: o.id });
    SM.save();
    return { ok: true, order: o };
  }
  ...
  /** Nhận cọc cho lượt đặt — qua D.thuTien để vào dòng tiền NGAY, không đợi tối mới thấy lệch. */
  function nhanCoc(t, bookingId, m) {
    const b = (t.bookings || []).find(x => x.id === bookingId);
    if (!b) return { ok: false, lyDo: 'Không tìm thấy lượt đặt' };
    if (b.state === 'cancelled') return { ok: false, lyDo: 'Lượt đặt đã huỷ — không nhận cọc thêm' };
    if (b.coc) return { ok: false, lyDo: 'Lượt này đã nhận cọc ' + SM.fmt.d(b.coc.soTien) + ' rồi — không ghi hai lần' };
    if (!(m.soTien > 0)) return { ok: false, lyDo: 'Số tiền cọc phải lớn hơn 0' };
    if (b.total && m.soTien > b.total) return { ok: false, lyDo: 'Tiền cọc vượt tổng tiền lượt đặt' };
    const hinhThuc = m.hinhThuc || 'tien-mat';
    const r = thuTien(t, { soTien: m.soTien, hinhThuc,
                           ghiChu: m.ghiChu || ('Cọc cho lượt đặt ' + b.id + ' — khách ' + b.guest),
                           loai: 'coc', veDatCho: b.id });
    if (!r.ok) return r;
    b.coc = { soTien: m.soTien, ngay: SM.CLOCK.today, hinhThuc, paymentId: r.phieu.id };
    SM.save();
    return { ok: true, coc: b.coc, phieu: r.phieu };
  }

  /* ───────── BỔ SUNG GIẤY TỜ BẢNG KÊ THIẾU (R2 #06) ───────── */

  /**
   * Bổ sung đúng trường thiếu của bảng kê thu mua — gặp lại người bán thì bổ
   * vào BẢNG KÊ CŨ chứ không lập mới (lập mới là đếm chi phí hai lần).
   * Chỉ cho bổ giấy tờ (CCCD, địa chỉ, ảnh biên nhận, ký nhận); số lượng và
   * đơn giá đã vào lô rồi thì không sửa đè ở đây.
   */
  function suaBangKe(t, id, m) {
    const p = (t.purchases || []).find(x => x.id === id);
    if (!p) return { ok: false, lyDo: 'Không tìm thấy bảng kê ' + id };
    if (!String((m || {}).lyDo || '').trim())
      return { ok: false, lyDo: 'Phải ghi lý do bổ sung — ví dụ «gặp lại ông ấy, chụp thêm biên nhận»' };
    const choSua = ['cccd', 'diaChi', 'anhBienNhan', 'kyNhan'];
    const truoc = {}, sau = {};
    choSua.forEach(f => {
      const v = (m || {})[f];
      if (v === undefined || v === null || v === '') return;   // trường nào không gửi thì giữ nguyên
      if (String(p[f]) === String(v)) return;                  // giá trị không đổi thì thôi
      truoc[f] = p[f] === undefined ? null : p[f];
      sau[f] = (f === 'anhBienNhan' || f === 'kyNhan') ? !!v : String(v).trim();
      p[f] = sau[f];
    });
    if (!Object.keys(sau).length) return { ok: false, lyDo: 'Chưa có trường nào được bổ sung' };
    p.boSungLuc = SM.CLOCK.today;
    p.boSungLyDo = String(m.lyDo).trim();
    nhatKy(t, { viec: 'Bổ sung giấy tờ bảng kê ' + id, doiTuong: id, truoc, sau, lyDo: p.boSungLyDo });
    SM.save();
    return { ok: true, rec: p, conThieu: D.checkPurchase(p).thieu };
  }

  Object.assign(D, {
    HINH_THUC_THU, thuTien, taoDonTay, tienChuaKhop, khopTienVaoDon, huyDon,
    nhanCoc, suaBangKe,
  });
})(window);
