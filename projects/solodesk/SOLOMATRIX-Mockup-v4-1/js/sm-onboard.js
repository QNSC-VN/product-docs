...
(function (global) {
  ...
  const THAM_SO_CONNECTOR = {
    ...
    zalooa: {
      ...
      phi: 'Trong 7 ngày khách vừa nhắn, được 8 tin trả lời miễn phí (tính trong 2 ngày); hết thì 55 đồng mỗi tin hỏi - trả lời, tin chốt đơn 165 đồng mỗi tin [Q-005]',
  ...
  function datKetNoi(t, ma, trangThaiMoi, meta) {
    if (typeof D.datTrangThaiKetNoi !== 'function')
      /* nhánh phòng hờ — hàm đã có từ W2; giữ lại để máy thiếu module domain vẫn nói lời thường chứ không chết câm */
      return { ok: false, lyDo: 'Máy này chưa có phần nối — nhờ cán bộ kỹ thuật xem giúp' };
  ...
  function moPhong(t, kichBanId, tenHien) {
    /* chi tiết kỹ thuật (tên module, việc W3) chỉ nằm comment — trước mặt hộ là lời thường (V5 #10, M3 #18) */
    if (!global.SM || !SM.inbox) { toast('Máy thiếu phần hộp thư — nhờ cán bộ kỹ thuật xem giúp'); return; }
    const r = SM.inbox.simulate(kichBanId, t.id);
    if (!r.ok) { toast('Chưa có kịch bản «' + tenHien + '» để chạy thử — ' + r.lyDo); return; }
    const pr = SM.inbox.process(r.suKien.id);
    if (!pr.ok) toast('Sự kiện đã vào hộp thư nhưng máy chưa xử lý được: ' + pr.lyDo);
  ...
  function bindObKichHoat(t) {
    ...
    if (xt) xt.onclick = () => {
      ...
      if (typeof SM.switchTenant !== 'function' || !SM.tenant('cd4-moi')) {
        /* thiếu seed hộ demo cd4-moi (đăng ký ở lượt build trước) — trước mặt hộ chỉ nói lời thường */
        toast('Máy này chưa có hộ mẫu để bấm thử — nhờ cán bộ kỹ thuật xem giúp');
        return;
      }
  ...
  function ganBindCau(t) {
    ...
    body.querySelectorAll('[data-tra]').forEach(b => {
      ...
      b.onclick = () => {
        ...
        if (b.closest('[data-chonroi]')) {
          /* câu 4 + V12 thêm câu 1/2/5: bấm nấc chỉ CHỌN — đổi được tới khi rời màn (W0 câu 4(d) sửa bấm nhầm) */
          b.parentNode.querySelectorAll('[data-tra]').forEach(x => x.classList.remove('pri'));
          b.classList.add('pri');
          /* V12 (R4): dòng dưới câu hỏi nói lại hộ vừa chọn gì — phản hồi ngay tại chỗ */
          const dem = b.closest('[data-chonroi]').querySelector('[data-chondem]');
          if (dem) dem.innerHTML = 'Đã chọn: <b>' + E(b.textContent.trim()) + '</b>';
          return;
        }
    ...
    /* V12 (R4): «Tiếp tục» của câu 1/2/5 — chưa chọn mà bấm thì nói bằng lời dẫn đường, không chặn câm */
    body.querySelectorAll('[data-tiep]').forEach(b => b.onclick = () => {
      const khoi = b.closest('[data-chonroi]');
      if (khoi && !khoi.querySelector('[data-tra].pri')) {
        toast('Bấm chọn một câu bên trên đã — bấm vào câu đúng với nhà mình, rồi bấm Tiếp tục');
        return;
      }
      ob.ob2.cau = ob.ob2.cau + 1; SM.save(); vongLaiCau(t);
    });
  ...
  function cauNganh(t, g) {
    ...
    const daChon = luc.find(x => x.gt === trangThai(t).traLoi.nganh);
    /* V12 (R4): chọn 1 chạm chỉ CHỌN — dòng dưới nói rõ đã chọn gì, bấm «Tiếp tục» mới sang bước (mẫu câu 4) */
    const than = '<div data-chonroi="1">' + luc.map(x =>
      '<button class="btn w ' + (daChon && daChon.gt === x.gt ? 'pri' : '') + '" style="margin-top:8px;text-align:left" data-cau="nganh" data-tra="' + x.gt + '">' +
      '<span style="font-size:20px;margin-right:8px">' + x.ic + '</span>' + x.ten + '</button>').join('') +
      '<div class="muted" style="margin-top:10px" data-chondem>' + (daChon ? 'Đã chọn: <b>' + daChon.ten + '</b>' : 'Chưa chọn — bấm nghề của nhà mình') + '</div>' +
      '<button class="btn w" style="margin-top:8px;font-size:17px" data-tiep>Tiếp tục</button></div>';
  ...
  function cauGiayTo(t) {
    const luc = [
      ...
      ['chua-co', 'Chưa đăng ký gì cả'], ['chua-ro', 'Không nhớ — để cán bộ xem giúp'],
    ];
    const daChon = luc.find(x => x[0] === trangThai(t).traLoi.giayTo);
    /* «Không nhớ» = đáp án AN TOÀN — nút to riêng, đừng ép hộ bấm bừa (W0 câu 2(d)).
       V12 (R4): hết pri cứng — pri giờ là dấu «đã chọn», bấm «Không nhớ» xong nút sáng như mọi đáp án */
    const than = '<div data-chonroi="1">' + luc.slice(0, 3).map(x =>
      '<button class="btn w ' + (daChon && daChon[0] === x[0] ? 'pri' : '') + '" style="margin-top:8px;text-align:left" data-cau="giayTo" data-tra="' + x[0] + '">' + x[1] + '</button>').join('') +
      '<button class="btn w ' + (daChon && daChon[0] === 'chua-ro' ? 'pri' : '') + '" style="margin-top:14px;font-size:17px;text-align:left" data-cau="giayTo" data-tra="chua-ro">Không nhớ — để cán bộ xem giúp</button>' +
      '<div class="muted" style="margin-top:10px" data-chondem>' + (daChon ? 'Đã chọn: <b>' + daChon[1] + '</b>' : 'Chưa chọn — bấm câu đúng với nhà mình') + '</div>' +
      '<button class="btn w" style="margin-top:8px;font-size:17px" data-tiep>Tiếp tục</button></div>';
  ...
  function cauPos(t) {
    const luc = [['giay-so', 'Ghi ra sổ, vở'], ['phan-mem-khac', 'Dùng app khác rồi'],
      ['may-tinh-tien', 'Có cái máy in bill ở quầy'], ['chua-ro', 'Không rõ']];
    const daChon = luc.find(x => x[0] === trangThai(t).traLoi.posHienTai);
    /* V12 (R4): câu cuối cũng theo mẫu câu 4 — chọn xong thấy rõ đã chọn gì rồi mới «Tiếp tục» */
    const than = '<div data-chonroi="1">' + luc.map(x =>
      '<button class="btn w ' + (daChon && daChon[0] === x[0] ? 'pri' : '') + '" style="margin-top:8px;text-align:left" data-cau="posHienTai" data-tra="' + x[0] + '">' + x[1] + '</button>').join('') +
      '<div class="muted" style="margin-top:10px" data-chondem>' + (daChon ? 'Đã chọn: <b>' + daChon[1] + '</b>' : 'Chưa chọn — bấm câu đúng với nhà mình') + '</div>' +
      '<button class="btn w" style="margin-top:8px;font-size:17px" data-tiep>Tiếp tục</button></div>';
  ...
  function sheetZaloDangKy(t) {
    sheet('Đăng ký Zalo OA ngay trong app',
      ...
      '<div class="note warn" style="margin-top:8px">Phí khi dùng: trong 7 ngày khách vừa nhắn, được 8 tin trả lời miễn phí (tính trong 2 ngày); hết thì mỗi tin hỏi - trả lời tốn 55 đồng, mỗi tin chốt đơn tốn 165 đồng.</div>' + /* [Q-005] */
      '</div></div>', w => {
  ...
  function dongHo30Ngay(t) {
    if (typeof D.mocVuotNguong !== 'function')
      /* nhánh phòng hờ — hàm đã có từ W2; máy thiếu module domain vẫn thấy lời thường chứ không thấy chữ việc build (V5 #10) */
      return 'Đồng hồ 30 ngày chưa chạy được trên máy này — nhờ cán bộ kỹ thuật xem giúp, đừng để trễ hạn.';
  ...
  function sheetDaXong(t, ma) {
    ...
    sheet(ts.ten + ' — đã nối',
      ...
      /* M3 #6 + quy ước R4: ngắt là hành động khó gỡ — nút đỏ, cách nút thường ≥16px, và luôn qua bước hỏi lại */
      '<button class="btn dan w" style="margin-top:16px" id="dx-ngat">Ngắt kết nối này</button>' +
      '</div></div>', w => {
        ...
        w.querySelector('#dx-ngat').onclick = () => {
          ...
          /* M3 #6: hỏi lại đúng TÊN + hậu quả bằng lời thường — bấm «Đồng ý ngắt» mới thật sự ngắt */
          sheet('Ngắt ' + ts.ten + '?',
            '<div class="card"><div class="bd">' +
            '<div style="font-size:15px;line-height:1.6">Ngắt rồi thì <b>' + E(ts.ten) + '</b> không tự gửi gì về máy nữa — đơn, tiền, tin từ chỗ này dừng lại tới khi nối lại. Sổ đã ghi sẵn thì vẫn nguyên đó, không mất gì.</div>' +
            '<button class="btn pri w" style="margin-top:12px" data-x>Để nguyên — không ngắt</button>' +
            '<button class="btn dan w" style="margin-top:16px" id="dx-ngatchac">Đồng ý ngắt ' + E(ts.ten) + '</button>' +
            '</div></div>', w2 => {
              w2.querySelector('#dx-ngatchac').onclick = () => {
