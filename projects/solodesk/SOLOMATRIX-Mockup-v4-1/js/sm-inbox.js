...
(function (global) {
  ...
  const NGUON = {
    shopee:  { ten: 'Shopee',            loai: 'Sàn thương mại điện tử', kenh: 'webhook', endpoint: 'POST /webhooks/ecommerce/orders', doTuoi: 'nguồn này chưa đo được' } /* nguồn: Q-036 */,
    tiktok:  { ten: 'TikTok Shop',       loai: 'Sàn thương mại điện tử', kenh: 'webhook', endpoint: 'POST /webhooks/ecommerce/orders', doTuoi: 'nguồn này chưa đo được' } /* nguồn: Q-046 */,
    lazada:  { ten: 'Lazada',            loai: 'Sàn thương mại điện tử', kenh: 'webhook', endpoint: 'POST /webhooks/ecommerce/orders', doTuoi: 'nguồn này chưa đo được' } /* nguồn: Q-046 */,
    ...
    booking: { ten: 'Booking.com',       loai: 'Nền tảng đặt phòng',     kenh: 'webhook', endpoint: 'POST /webhooks/booking/reservations', doTuoi: 'nguồn này chưa đo được' } /* nguồn: Q-049 */,
    zalo:    { ten: 'Zalo OA',           loai: 'Kênh chạm khách',        kenh: 'webhook', endpoint: 'POST /webhooks/zalo/messages', doTuoi: 'nguồn này chưa đo được' } /* nguồn: Q-034 */,
    sepay:   { ten: 'SePay — ngân hàng', loai: 'Dòng tiền',              kenh: 'webhook', endpoint: 'POST /webhooks/sepay', doTuoi: 'nguồn này đẩy tức thì — giây đến phút' } /* nguồn: Q-006 */,
    ghn:     { ten: 'Giao Hàng Nhanh',   loai: 'Vận chuyển',             kenh: 'webhook', endpoint: 'POST /webhooks/shipping/ghn', doTuoi: 'nguồn này chưa đo được' } /* nguồn: Q-037 */,
    vtp:     { ten: 'Viettel Post',      loai: 'Vận chuyển',             kenh: 'webhook', endpoint: 'POST /webhooks/shipping/vtp', doTuoi: 'nguồn này chưa đo được' } /* nguồn: Q-037 */,
    cqt:     { ten: 'Cơ quan thuế',      loai: 'Tuân thủ',               kenh: 'trả lời đồng bộ', endpoint: 'phản hồi cho POST /einvoice/issue', doTuoi: 'trả lời sau 15 phút đến 1 ngày làm việc' } /* nguồn: Q-002 */,
    'cts-ncc': { ten: 'Nhà cung cấp chữ ký số', loai: 'Chữ ký số',       kenh: 'webhook', endpoint: 'POST /webhooks/cts/status', doTuoi: 'nguồn này chưa đo được' },
  };
  ...
  function process(evId) {
    ...
    if (e.loaiSuKien === 'don-moi') {
      ...
      SM.save();
      // V5 #5: nói bằng lời thường, tránh hiểu ngược thành «app nộp thuế thay» (rủi ro N-06):
      // sàn đã trừ sẵn trong tiền về và nộp hộ — nhà mình KHÔNG nộp thêm lần nữa.
      taoRa = { loai: 'don', id: o.id, moTa: 'Đơn hàng mới, đã giữ chỗ tồn kho' +
        (p.thueSanDaNop ? ' — sàn đã trừ sẵn tiền thuế ' + SM.fmt.d(p.thueSanDaNop) +
          ' trong tiền về và nộp hộ rồi, nhà mình không nộp thêm lần nữa (số do sàn gửi kèm đơn)' : '') };
    }

    else if (e.loaiSuKien === 'tien-ve') {
      t.payments = t.payments || [];
      const pm = { date: SM.CLOCK.today, point: p.diemQuet || (t.qrPoints || [{ id: 'q1' }])[0].id,
                   amount: p.soTien, method: 'chuyen-khoan', noiDung: p.noiDung, tuSuKien: e.id };
      // khớp đơn nếu nội dung chuyển khoản có mã đơn
      let khop = null;
      (t.orders || []).forEach(o => { if (p.noiDung && p.noiDung.indexOf(o.id) >= 0) khop = o; });
      if (khop) { khop.state = 'paid'; khop.paidAt = SM.CLOCK.today; pm.orderId = khop.id; }
      else pm.choKhop = true;   // F2 R2 #03: tiền về không có mã đơn — giữ lại cho màn khớp tay (D.tienChuaKhop)
      t.payments.push(pm);
      SM.save();
      taoRa = { loai: 'thanh-toan', id: p.maGiaoDich,
                moTa: khop ? ('Đã khớp và đánh dấu đã thu tiền cho đơn ' + khop.id)
                           : 'Ghi nhận tiền về — máy để sẵn trong mục khớp tiền, cô chú gán cho đúng đơn giúp máy' };
    }
      ...
      if (p.trangThai === 'da-giao') {
        ...
        if (o.cod) { t.payments = t.payments || [];
          t.payments.push({ date: SM.CLOCK.today, point: (t.qrPoints || [{ id: 'q1' }])[0].id,
                            amount: o.cod, method: 'Tiền thu hộ', noiDung: 'COD đơn ' + o.id,
                            orderId: o.id, tuSuKien: e.id }); }   // F2 R2 #03: đã biết thuộc đơn nào — khỏi lọt màn khớp tay
      }
      SM.save();
  ...
  const KICH_BAN = [
    { id: 'sanTMDT', ten: 'Khách đặt hàng trên sàn thương mại điện tử', nguonId: 'shopee',
      ...
      dung: t => {
        ...
        // V5 #5: lời thường — sàn trừ sẵn thuế trong tiền về và nộp hộ, nhà mình không nộp thêm lần nữa
        return { loaiSuKien: 'don-moi', tieuDe: 'Đơn mới từ Shopee — ' + s.meta.name + ' ×' + qty +
                 ' — sàn đã trừ sẵn tiền thuế ' + SM.fmt.d(thueNopThay) + ', nhà mình không nộp thêm lần nữa',
    ...
    { id: 'ket-noi-dut', ten: 'Kênh sàn im lặng — kết nối có thể đứt', nguonId: 'shopee',
      moTa: 'Sàn hết đẩy đơn qua 24 giờ — bước đối soát hằng ngày phát hiện và lên tiếng (app tự hỏi lại ít nhất mỗi ngày một lần).',
    ...
    { id: 'cts-duyet-xong', ten: 'Chữ ký số của hộ kích hoạt xong', nguonId: 'cts-ncc',
      moTa: 'F2 R2 #19 (chốt P1): đường ra của cảnh báo «hoá đơn đang chờ chữ ký số» — kích hoạt xong là phát hành hoá đơn được ngay, bán vẫn ghi bình thường trong lúc chờ.',
      apDung: t => trangThaiKetNoi(t, 'cts') !== 'da_ket_noi',
      dung: () => ({ loaiSuKien: 'ket-noi-duyet', tieuDe: 'Chữ ký số đã kích hoạt — xuất hoá đơn được ngay',
        payload: { kenh: 'cts', ketQua: 'chap-nhan', aiBam: 'nhà cung cấp chữ ký số (mô phỏng)',
                   luc: SM.CLOCK.today + 'T10:00:00' } }) },
  ];
