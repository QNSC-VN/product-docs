 ...
 *
 * Nâng cấp V3 (20/08/2026) — 48 hộ mô phỏng PHẢI SINH ĐƯỢC VIỆC cho cán bộ:
 * seed giờ mang ketNoi chi tiết + viecHoanLuat + vuotLuc cùng shape hộ thật.
 * Mọi khối quét việc (hạn 30 ngày · chờ quá hạn · hoãn luật · việc hôm nay)
 * đọc HAI LOẠI HỘ qua MỘT hàm chuẩn hoá duy nhất `chuanHoaHo` — không nhánh
 * song song cho từng loại (chính là lỗ W9: hộ thật có shape này, hộ mô phỏng
 * chỉ có ketNoiTom đếm theo trạng thái nên KPI «Việc hôm nay» trống).
 *
...
(function (global) {
  ...
  /** V3 — MỘT hàm chuẩn hoá duy nhất cho MỌI hộ (thật trong kho lẫn mô phỏng của seed).
      Mọi khối Sổ trực (hạn 30 ngày · chờ quá hạn · hoãn luật · việc hôm nay · KPI ·
      định mức) chỉ đọc shape này — mở nhánh song song nơi khác cho riêng một loại hộ là lỗi.
      goc đã là bản trích từng nguồn (đường đọc khác nhau), phần SUY giống nhau tập trung ở đây. */
  function chuanHoaHo(g) {
    let phut = lay(g, ['phutTuKichHoat', 'phut']);
    if (phut === null && g.viecDauTien && g.viecDauTien.doneLuc && g.viecDauTien.batDauLuc)
      phut = soPhut(g.viecDauTien.batDauLuc, g.viecDauTien.doneLuc);
    const ketNoi = knDict(g.ketNoi);
    /* hoãn nhóm LUẬT: hộ thật lẫn seed đều ghi mảng {ma, ngay, nguoi, lyDo} (ob.viecHoanLuat);
       giữ thêm nhánh đọc trangThai 'hoan' cũ trong ketNoi để không mất dữ liệu đã ghi kiểu đó */
    const hoanLuat = (g.hoanLuat || []).slice();
    Object.keys(ketNoi).forEach(ma => {
      if (ketNoi[ma] && ketNoi[ma].trangThai === 'hoan' && !hoanLuat.some(v => v.ma === ma))
        hoanLuat.push({ ma, ngay: lay(ketNoi[ma], ['batDauLuc']) || null, nguoi: null,
                        lyDo: lay(ketNoi[ma], ['hoanLyDo', 'lyDoHoan', 'lyDo']) || null });
    });
    return {
      that: !!g.that,
      dayDu: g.dayDu !== false,                  // W9-vá số: tenant demo trắng chưa vào theo dõi
      id: g.id || g.ma || null,
      ma: g.ma || null,
      ten: g.ten || '—', diaBan: g.diaBan || '—', nganh: g.nganh || '—',
      buoc: g.buoc || null, canBo: g.canBo || null,
      cungCanBo: !!g.cungCanBo,                  // định mức: có cán bộ NGỒI CẠNH hay hộ tự làm
      vuotLuc: g.vuotLuc || null,
      ketNoi, hoanLuat,
      nhatKy: g.nhatKy || [],
      phut: phut === null ? null : Math.max(1, Math.round(phut)),
      dot: g.dot || (ketNoi.zalooa ? lay(ketNoi.zalooa, ['dot']) : null) || null,
      onb: g.onb || null,                        // C2: dữ liệu «dở ở câu mấy» — chỉ hộ thật có
    };
  }
  ...
  function hoThat() {
    return SM.tenantIds().map(id => {
      ...
      return chuanHoaHo({
        ...
        id: t.id, ma: (t.license && t.license.ma) || null,
        ten: t.name, diaBan: t.diaBan, nganh: t.nganh,
        buoc: ob.buoc || null,
        canBo: ob.coCanBo ? ob.coCanBo.maCanBo : null,
        cungCanBo: !!(ob.coCanBo && ob.coCanBo.maCanBo),
        vuotLuc: t.vuotLuc || null,
        ketNoi: ob.ketNoi, hoanLuat: ob.viecHoanLuat || [],
        viecDauTien: ob.viecDauTien,
        nhatKy: t.nhatKy || [],
        dot: lay(t, ['dotNopOA']) || null,
  ...
  /** 48 hộ mô phỏng (W4) — trích đường đúng của seed rồi qua chuanHoaHo; seed chưa có thì trả [] */
  function hoMoPhong() {
    ...
    return ds.map((r, i) => {
      ...
      return chuanHoaHo({
        ...
        cungCanBo: lay(r, ['cungCanBo']) === true,
        vuotLuc: lay(r, ['vuotLuc']),
        ketNoi: lay(r, ['ketNoi']) || ob.ketNoi,
        hoanLuat: lay(r, ['viecHoanLuat', 'hoanLuat']) || [],
        viecDauTien: lay(r, ['viecDauTien']) || ob.viecDauTien || {},
        nhatKy: lay(r, ['nhatKy']) || [],
        phutTuKichHoat: lay(r, ['phutTuKichHoat', 'phut']),
        dot: lay(r, ['dotNopOA', 'dotOA', 'dot']) || null,
      });
    });
  }
  ...
  function viecCuaHo(h) {
    ...
    (h.hoanLuat || []).forEach(v => {
      if (!v || !v.ma) return;
      ds.push({ muc: 'warn', loai: 'Hoãn việc nhóm LUẬT', khoa: h.id + '|hoan|' + v.ma, tenKN: tenKN(v.ma),
                text: 'Hẹn lại ' + h.ten + ' — đang hoãn ' + tenKN(v.ma) + (v.lyDo ? ' (lý do: ' + v.lyDo + ')' : '') + '. Việc luật không tự mất, cán bộ theo tới cùng' });
    });
    return ds;
  }
  ...
  function phanBoPhut(hs) {
    ...
    hs.forEach(h => {
      ...
      /* V3: nhóm theo «có cán bộ NGỒI CẠNH» (cungCanBo) — hộ mô phỏng nào cũng CÓ cán bộ
         địa bàn gán, nhóm theo canBo sẽ dồn cả 48 hộ vào một cột và cột «tự làm» trống sai */
      if (h.cungCanBo) { cb[i]++; tcb += h.phut; ncb++; } else { tl[i]++; ttl += h.phut; ntl++; }
    });
    return {
      nhan, cb, tl,
  ...
  function khungHan30(hs) {
    ...
    if (!dong.length) h += '<tr><td colspan="5">' + (seedSan() ? 'Không có hộ nào đang đếm lùi hạn 30 ngày.' : 'Đang nạp dữ liệu…') + '</td></tr>';
      ...
      /* V3: hộ đã nối hoá đơn điện tử xong thì nghĩa vụ 30 ngày đã đáp ứng —
       hiện «đã đăng ký» thay vì đếm lùi/đỏ gây hiểu hộ còn nợ việc */
      const daDangKy = x.h.ketNoi.hddt && x.h.ketNoi.hddt.trangThai === 'da_ket_noi';
      const cl = daDangKy ? tag('ok', 'đã đăng ký hoá đơn điện tử')
        : x.m.chuaDenLuc ? tag('no', 'kỳ thuế chưa chốt')
  ...
  function khungHoanLuat(hs) {
    /* V3: đọc mảng hoanLuat do chuanHoaHo gom (ob.viecHoanLuat của hộ thật + seed) —
       trước đây đọc ketNoi[ma].trangThai 'hoan' mà không đường nào ghi trạng thái đó */
    const dong = [];
    hs.forEach(h => (h.hoanLuat || []).forEach(v => {
      if (!v || !v.ma) return;
      dong.push({ h, ma: v.ma, lyDo: v.lyDo || '(chưa ghi lý do)', tuLuc: v.ngay || null });
    }));
    ...
    body += mas.length
      ? '<div class="scrollx" style="border:none"><table><thead><tr><th>Việc đang nối</th><th>Trạng thái</th><th>Ngày gửi</th><th>Dự kiến / ghi chú</th></tr></thead><tbody>'
        + mas.map(ma => {
          ...
          const hoanCua = (h.hoanLuat || []).find(v => v.ma === ma);
          const ghiChu = k.hanDuKien ? 'dự kiến ' + E(F.dmy(k.hanDuKien))
            : hoanCua ? 'đang hoãn' + (hoanCua.lyDo ? ': ' + E(hoanCua.lyDo) : '')
            : (k.lyDo ? E(k.lyDo) : '—');
  ...
  global.SM.b2g = {
    ...
    chuanHoaHo,            // V3: hàm chuẩn hoá duy nhất — mọi khối đọc shape này
  };
