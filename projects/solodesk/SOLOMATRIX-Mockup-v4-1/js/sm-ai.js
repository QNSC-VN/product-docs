...
(function (global) {
  ...
  const KB = {
    ...
    bai: [
      ...
      { id: 'KB-02', pheDuyet: false,
        ...
        traLoi: 'Hộ kinh doanh có doanh thu trên 01 tỷ đồng một năm thuộc diện sử dụng hoá đơn điện tử khởi tạo từ máy tính tiền, có kết nối chuyển dữ liệu với cơ quan thuế. Ứng dụng theo dõi doanh thu luỹ kế và cảnh báo trước khi hộ chạm mốc 1 tỷ đồng, để hộ chuẩn bị chứ không bị động.',
      ...
      { id: 'KB-05', pheDuyet: false,
        ...
        traLoi: 'Nền tảng chuẩn bị số liệu, lập tờ khai nháp, soát lỗi và dẫn hộ sang ứng dụng thuế điện tử ngay trên điện thoại. Nhưng người chịu trách nhiệm phải tự bấm nộp trên cổng của cơ quan thuế. Nền tảng chỉ lưu lại mã biên nhận do hộ nhập vào. Đây là ranh giới trách nhiệm chủ đích, không phải thiếu tính năng: tờ khai có giá trị pháp lý thì người ký phải là người chịu trách nhiệm.',
        nguon: 'Ranh giới trách nhiệm của nền tảng — mục giới hạn cứng trong tài liệu vận hành của Chương trình' },
      ...
      { id: 'KB-07', pheDuyet: false,
        ...
        traLoi: 'Doanh thu mùa vụ không làm đổi cách khai: vẫn khai theo doanh thu thực tế từng kỳ. Điều cần chuẩn bị là dòng tiền — quý cao điểm phát sinh số thuế lớn, nên phải để dành từ trong mùa. Ứng dụng hiện doanh thu theo tháng, tính số thuế tạm tính luỹ kế ngay từng ngày và nhắc trước mốc, để hộ không bị bất ngờ ở quý cao điểm.',
  ...
  const CHUA_DO = 'chưa đo được'; /* nguồn: Q-036/Q-037 — radar đang hỏi, chưa có số công bố */
  ...
  const A_HANDLERS = [
    ...
    },
    {
      ...
      run(t) {
        ...
        const rows = e.byGroup.map(g => g.rate
          ? `${g.rate.ten}: doanh thu ${F.d(g.revenue)} — thuế giá trị gia tăng ${F.pct(g.rate.gtgt)} là ${F.d(g.gtgt)}, thuế thu nhập cá nhân ${F.pct(g.rate.tncn)} là ${F.d(g.tncn)} (ghi ${g.book})`
          : `${g.group}: ${F.d(g.revenue)}`);
        return {
          chinh: e.regime === 'dn'
            ? `Quý ${SM.CLOCK.quarter}/${SM.CLOCK.year} tạm tính phải nộp ${F.d(e.total)} — gồm thuế giá trị gia tăng (GTGT) ${F.d(e.gtgt)} và thuế thu nhập doanh nghiệp (TNDN) ${F.d(e.tndn)} (thuế suất ${e.tndnSuat}%).`
            : `Quý ${SM.CLOCK.quarter}/${SM.CLOCK.year} tạm tính phải nộp ${F.d(e.total)} — gồm thuế giá trị gia tăng (GTGT) ${F.d(e.gtgt)} và thuế thu nhập cá nhân (TNCN) ${F.d(e.tncn)}.`,
          chiTiet: rows.concat(e.mienThue ? ['Doanh thu năm chưa vượt mốc miễn thuế nên tạm tính bằng 0.'] : []),
    ...
    },
    {
      ...
      run(t) {
        ...
        return {
          chinh: full.length
            ? `Có ${full.length} khung đã đầy trong 3 ngày tới — không đặt thêm được vào khung đó.`
            : 'Ba ngày tới còn chỗ ở tất cả các khung đang có khách.',
          chiTiet: lines.length ? lines : ['Chưa có lượt đặt nào trong 3 ngày tới.'],
          tinhTu: 'Cộng số khách đã đặt theo từng chỗ cho thuê và từng khung giờ, so với số chỗ tối đa.',
        };
      }
    },
    {
      ...
      run(t) {
        ...
        return {
          ...
          tinhTu: 'Soát từng bảng kê thu mua theo danh mục giấy tờ bắt buộc.',
        };
      }
    },
    {
    ...
    {
      key: ['co phai dung may tinh tien', 'vuot 1 ty', 'nguong doanh thu', 'phai lam hoa don', 'dang ky hoa don'],
      run(t) {
        const p = D.needsPosInvoice(t);
        return {
          chinh: p.thuocDien
            ? `Đã thuộc diện dùng hoá đơn điện tử khởi tạo từ máy tính tiền: doanh thu từ đầu năm ${F.d(p.yearRev)}, vượt mốc ${F.dShort(p.nguong)}.`
            : p.sapVuot
              ? `Chưa vượt, nhưng đang trên đà vượt: đầu năm tới nay ${F.d(p.yearRev)}, theo tiến độ này cả năm khoảng ${F.d(p.projected)} — trên mốc ${F.dShort(p.nguong)}. Nên chuẩn bị trước.`
              : `Chưa thuộc diện: doanh thu từ đầu năm ${F.d(p.yearRev)}, dự kiến cả năm khoảng ${F.d(p.projected)}, dưới mốc ${F.dShort(p.nguong)}.`,
          chiTiet: [`Mốc áp dụng: ${D.TAX.nguong.posInvoice.ten}`],
    ...
    {
      key: ['don moi nhat', 'don vua ve', 'don cuoi cung', 'don moi ve luc nao', 'co ai chuyen tien chua', 'tien ve chua', 'tien ve luc nao'],
      run(t) {
        if (!SM.inbox || !SM.inbox.list) return null;
        // list() xếp mới nhất trước — lấy tin mới nhất trong hai loại: đơn mới hoặc tiền về
        const ev = SM.inbox.list(t.id).find(e => e.loaiSuKien === 'don-moi' || e.loaiSuKien === 'tien-ve');
        if (!ev) return {
          chinh: 'Chưa có đơn nào và cũng chưa có khoản tiền nào về trong hộp thư của app.',
          chiTiet: ['Muốn xem trực tiếp, cô chú mở mục «Hộp thư đến» trong app — tin nào tới thì nằm ngay trong đó.'],
          tinhTu: 'Hộp thư trong app ghi lại từng tin các kênh gửi tới, kèm giờ tới.',
        };
        const laTien = ev.loaiSuKien === 'tien-ve';
        const p = ev.payload || {};
        const tong = (p.items || []).reduce((s, it) => s + (it.qty || 0) * (it.donGia || 0), 0);
        const nguon = SM.inbox.nguonCua(ev);
        const xuLy = ev.trangThai === 'da-xu-ly' ? (laTien ? 'đã vào sổ thu' : 'đã vào sổ bán')
          : ev.trangThai === 'loi' ? 'bị kẹt — cần cô chú xem lại'
          : ev.trangThai === 'trung-bo' ? (laTien ? 'bản trùng theo mã — đã bỏ, không cộng tiền lần hai' : 'bản trùng theo mã — đã bỏ, không cộng lần hai')
          : 'chờ cô chú mở ra xem';
        return {
          chinh: laTien
            ? `Khoản tiền mới nhất về lúc ${F.hm(ev.luc)} ngày ${F.dmy(String(ev.luc).slice(0, 10))}: ${F.d(p.soTien || 0)}${p.noiDung ? `, nội dung «${p.noiDung}»` : ''}.`
            : `Đơn mới nhất về lúc ${F.hm(ev.luc)} ngày ${F.dmy(String(ev.luc).slice(0, 10))}, từ kênh ${nguon.ten}.`,
          chiTiet: [
            laTien ? (p.maGiaoDich ? `Mã giao dịch ${p.maGiaoDich}.` : null)
                   : (p.maDon ? `Mã đơn ${p.maDon}, khách ${p.khach || 'chưa rõ tên'}.` : null),
            laTien ? (p.soTien ? `Số tiền ${F.d(p.soTien)} — ${xuLy}.` : `Tình trạng: ${xuLy}.`)
                   : (tong ? `Giá trị ${F.d(tong)} — ${xuLy}.` : `Tình trạng: ${xuLy}.`),
          ].filter(Boolean),
          tinhTu: 'Hộp thư trong app ghi lại từng tin NGAY KHI nó tới: bên nào gửi, giờ nào tới, tin nói gì.',
        };
      }
    },
    {
      key: ['kenh nao dut', 'ket noi on khong', 'cac kenh co on khong', 'kenh chet', 'truc trac', 'kenh nao loi'],
      run(t) {
        ...
        return {
          ...
          tinhTu: 'Mỗi chỗ nối tự báo hiệu; chỗ nào im quá lâu thì app đánh dấu đứt và nhắc kiểm tra lại.',
        };
      }
    },
    {
      key: ['du lieu tuoi', 'du lieu tuoi toi dau', 'cap nhat lan cuoi', 'so moi ve'],
      run(t) {
        const cs = D.connectors(t).filter(c => c.trangThai ? c.trangThai !== 'chua-noi' : c.noi);
        if (!cs.length) return null;
        // dòng «chưa đo…» (tiền tố chung với sm-domain/sm-onboard) không tính là đã đo
        const chuaDo = cs.filter(c => {
          const cb = (c.doTuoi || {}).congBo || '';
          return !cb || cb.indexOf('chưa đo') === 0;
        }).length;
        const soDo = cs.length - chuaDo;
        return {
          chinh: chuaDo
            ? `Nhà mình đang nối ${cs.length} chỗ. ${soDo} chỗ app biết rõ dữ liệu mới tới lúc nào, ${chuaDo} chỗ «${CHUA_DO}» — chưa có bằng chứng thì app không nói bừa.`
            : `Nhà mình đang nối ${cs.length} chỗ — chỗ nào app cũng biết rõ dữ liệu mới tới lúc nào.`,
          chiTiet: cs.map(c => {
            ...
            return `${c.ten}: ${tuoi}${phut !== null ? ` — app nhận số mới cách đây ${phut} phút` : ''}`;
          }),
          tinhTu: 'Từng chỗ nối đều ghi sẵn: dữ liệu tới bằng cách nào, app kiểm được tới đâu, cộng mốc lần cuối nhận số mới của chính chỗ đó.',
        };
      }
    },
  ];
  ...
  function afterHoursReply(t, req) {
    ...
    return {
      guiLuc: 'Ngoài giờ làm việc — app soạn sẵn câu trả lời, cô chú đọc rồi bấm gửi, tin mới đi',
      than: [
      ...
      ranhGioi: 'App chỉ báo chỗ trống và giữ tạm. Chốt đơn và nhận tiền vẫn do cô chú quyết.',
  ...
  /* Thời gian trả lời đầu cho câu hỏi nghiệp vụ — lấy từ bảng cam kết phía trên, không viết cứng. */
  function slaTraLoiDau() {
    const row = SLA.doTre.filter(x => x.muc.indexOf('Hỏi nghiệp vụ') === 0)[0];
    return row ? row.dauTien : 'trong ngày làm việc';
  }
  ...
  function ask(question, tenant) {
    ...
    return Object.assign({ lop: 'C', nhan: 'Trợ lý không tự trả lời — đã chuyển người thật', hoi: question,
      chinh: 'Câu này app chưa có số liệu để trả lời ngay — app không đoán bừa. App đã ghi lại và chuyển cho cán bộ hỗ trợ: cô chú xem phiếu bên dưới, trả lời đầu tiên trong ' + slaTraLoiDau() + '.' },
  ...
  function suggestions(t) {
    ...
    const extra = [];
    // 3 câu hỏi theo lời hộ thật sự hỏi — đặt đầu để luôn hiện trong 8 chip mobile hiển thị
    extra.push('Có ai chuyển tiền chưa?');
    extra.push('Kênh nào đang trục trặc?');
    extra.push('Bao giờ phải làm hoá đơn?');
