...
(function (global) {
  ...
  /** Số máy tính được trong ca: tách tiền trong NGĂN KÉO và tiền vào TÀI KHOẢN.
      F2 20/08: nhận cả mã hình thức mới do D.thuTien ghi ('tien-mat', 'qr',
      'chuyen-khoan', 'the'); tiền thu hộ của hãng vận chuyển về TÀI KHOẢN của hộ
      (bảng C.15 — mục Đơn vị vận chuyển) nên không còn cộng vào ngăn kéo. */
  const TRONG_NGAN_KEO = ['Tiền mặt', 'tien-mat'];
  const VAO_TAI_KHOAN = ['QR', 'qr', 'chuyen-khoan', 'the', 'Tiền thu hộ'];
  function tinhCa(t, ca) {
    const pm = (t.payments || []).filter(p => p.date === ca.ngay);
    const tienMat = pm.filter(p => TRONG_NGAN_KEO.indexOf(p.method) >= 0).reduce((s, p) => s + p.amount, 0);
    const qua = pm.filter(p => VAO_TAI_KHOAN.indexOf(p.method) >= 0).reduce((s, p) => s + p.amount, 0);
    // R2 #05: cọc TIỀN MẶT nhận trong ngày mà đường cũ ghi thẳng b.coc (chưa qua
    // dòng tiền) vẫn nằm trong ngăn kéo — không cộng là tối đối tiền lệch đúng số cọc.
    // Cọc đã ghi dòng tiền (paymentId) hoặc đã xử lý xong (daXuLyTien) thì khỏi cộng.
    const cocMat = (t.bookings || [])
      .filter(b => b.coc && b.coc.ngay === ca.ngay && !b.coc.paymentId && !b.coc.daXuLyTien &&
                   (b.coc.hinhThuc || 'tien-mat') === 'tien-mat')
      .reduce((s, b) => s + b.coc.soTien, 0);
    const hd = (t.invoices || []).filter(i => i.date === ca.ngay).length;
    return { tienMat: tienMat + cocMat, chuyenKhoan: qua, tong: tienMat + cocMat + qua, soHoaDon: hd,
             duKienTrongKet: (ca.tienDauCa || 0) + tienMat + cocMat };
  }
  ...
  function traNoMotPhan(t, id, soTien, hinhThuc) {
    ...
    t.payments.push({ date: SM.CLOCK.today, point: (t.qrPoints || [{ id: 'q1' }])[0].id,
                      ...
                      veKhoanNo: r.id });   // F2 R2 #03: tiền thu nợ — khỏi lọt màn khớp tiền-đơn
  ...
  function dieuChinhHoaDon(t, invId, m) {
    ...
    // F2 R2 #19 (chốt P1): chưa nối chữ ký số thì không phát hành hoá đơn mới dưới
    // mọi hình thức — kể cả điều chỉnh/thay thế.
    const chan = D.chanPhatHanh ? D.chanPhatHanh(t) : null;
    if (chan) return chan;
  ...
  function noShow(t, bookingId, m) {
    ...
    if (b.coc) {
      const daGhiDongTien = !!b.coc.paymentId;   // cọc qua D.nhanCoc — đã nằm trong payments
      b.cocXuLy = giu ? 'giữ cọc' : 'hoàn cọc';
      if (giu) {
        if (!daGhiDongTien) {
          // đường cũ: cọc chưa từng vào sổ, giờ giữ lại thì ghi thu MỘT lần
          t.payments = t.payments || [];
          t.payments.push({ date: SM.CLOCK.today, point: (t.qrPoints || [{ id: 'q1' }])[0].id,
                            amount: b.coc.soTien, method: 'Tiền mặt', noiDung: 'Giữ cọc do khách không đến — ' + b.id,
                            veDatCho: b.id });
        }
        // đã ghi dòng tiền rồi thì tiền vốn đã nằm trong sổ — KHÔNG ghi thêm lần hai
      } else {
        // hoàn cọc (R2 #05): tiền trả lại khách — ghi DÒNG TRỪ để sổ ca còn đúng
        t.payments = t.payments || [];
        t.payments.push({ date: SM.CLOCK.today, point: (t.qrPoints || [{ id: 'q1' }])[0].id,
                          amount: -b.coc.soTien, method: b.coc.hinhThuc || 'Tiền mặt',
                          noiDung: 'Hoàn cọc cho khách — ' + b.id, veDatCho: b.id });
      }
      b.coc.daXuLyTien = true;   // các chỗ đếm cọc rời (tinhCa, qrPoints) dừng đếm — tiền giờ thể hiện qua payments
    }
