...
| US/DE status contract | Schedule State and Flow State both default to `Idea`, mirror bidirectionally and use `Idea/Defined/In-Progress/Completed/Accepted/Release`; Task and Defect State retain separate catalogs. |
| Phase 2 scope | Phase 2 includes Backlog Enhancement, Timeboxes/Iterations and Iteration Status. |
| Iteration lifecycle | Iteration uses `Planning -> Committed -> Accepted`. Assignment never auto-commits; an authorized user commits scope manually; `Committed` does not lock scope and there is no dedicated carry-over workflow. |
| Task roll-up | Completing all child Tasks auto-completes parent Story/Defect to `Completed`; reopening a Task from that all-completed state recalculates metrics and auto-moves the parent to `In-Progress`; initial partial completion does not auto-complete parent. |
...
| Permission | WA is the only company authority; Admin/Editor resolve independently per Project; a user with no Project row has implicit No Access. UI gating is UX only; backend still enforces every action and scope. |
