...
- Team is optional in Phase 2. Blank Team means Project-level / Project backlog scope.
- `All Teams` is not required for Phase 0-4 Iteration Status; mark any top-level All Teams expectation as Future Backlog unless BA reopens it.
...
|---|---|---|---|---|---|
| P2-CTX-001 | P0 | Context filters Backlog | Select Project A and optional Team A; open Backlog | Backlog shows only Story/Defect for selected Project and matching Team scope | Not Run |
| P2-CTX-002 | P0 | Context filters Timeboxes | Select Project A and optional Team A; open Plan -> Timeboxes | Iterations shown belong only to selected Project and matching Team scope | Not Run |
| P2-CTX-003 | P0 | Context filters Iteration Status selector | Select Project A and optional Team A; open Track -> Iteration Status | Iteration selector only lists Iterations for selected Project and matching Team scope | Not Run |
...
| P2-CTX-005 | P1 | Project backlog context | Select Project with blank Team | Backlog, Timeboxes and Iteration Status support Project-level / Project backlog scope | Not Run |
...
| P2-BL-008 | P0 | Invalid iteration assignment rejected | Assign item to Iteration from different Project or mismatched Team scope | Validation rejects assignment | Not Run |
...
| P2-BL-013 | P1 | Retired: project-wide read-only tier (Backlog) | N/A — no project-wide read-only tier exists in the 3-level model (Workspace Admin / Admin / Editor); both assignable levels can edit | Rows are readable; inline edit, bulk assign and reorder are absent/rejected | N/A (Viewer level removed; access model is now 3-level: Workspace Admin / Admin / Editor) |
...
| P2-IT-007 | P0 | Create Iteration modal | Click Create Iteration | Modal opens with Type, Project, optional Team, Name, Start Date, End Date, State | Pass (M5.2 runtime) |
| P2-IT-008 | P0 | Create Iteration auto-fills context | Select Project and optional Team; open create modal | Project defaults from active context; Team may remain blank for Project backlog scope | Not Run |
...
| P2-IT-017 | P1 | Retired: project-wide read-only tier (Timeboxes) | N/A — no project-wide read-only tier exists in the 3-level model (Workspace Admin / Admin / Editor); both assignable levels can create/edit Iterations | Iterations are readable; create/edit controls are absent and API rejects mutation | N/A (Viewer level removed; access model is now 3-level: Workspace Admin / Admin / Editor) |
...
| P2-IS-005 | P0 | Selector filtered by context | Switch Project/Team or leave Team blank | Selector options change to matching Iterations only | Not Run |
...
| P2-IS-011 | P1 | Required columns | Inspect list | Columns include checkbox, rank, ID, Type, Name, Schedule State, Flow State, Iteration, Blocked, Plan Est, Task Est, To Do, Owner | Not Run |
...
| P2-IS-015 | P0 | Inline edit supported fields | Edit Name, Schedule State, Flow State, Iteration, Plan Est, Owner | Changes persist with validation and permission enforcement | Not Run |
...
| P2-IS-023 | P1 | Retired: project-wide read-only tier (Iteration Status) | N/A — no project-wide read-only tier exists in the 3-level model (Workspace Admin / Admin / Editor); both assignable levels can edit | Inline edit and Add Item are absent; API rejects mutation | N/A (Viewer level removed; access model is now 3-level: Workspace Admin / Admin / Editor) |
...
1. Select Project and optional Team context.
2. Open Backlog and create Story/Defect.
3. Open Timeboxes and create Iteration for the same Project and matching Team scope.
...
9. Switch Project/Team or blank Team and confirm the previous Iteration/items obey the matching scope.
