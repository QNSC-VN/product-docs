...
> **Current permission note (2026-08-13):** Role Badge entries are legacy library inventory, not the current authorization taxonomy. Use `Workspace Admin` for the internal company owner and `Admin`/`Editor` as contextual Project access values (No Access is implicit — no `project_members` row).
