...
- Workspace selector Project and optional Team context controls Phase 2 data visibility. Blank Team means Project-level / Project backlog scope.
- Create Work Item and Create Iteration auto-fill Project from the selected context; Team may remain blank when the item/timebox belongs to Project backlog scope.
- Current mock account is Workspace Admin, so admin may switch Project/Team in create/edit forms where enabled; final Team must still belong to the selected Project when supplied.

BA decision 2026-07-06:

- `All Teams` is not required for Phase 0-4 Iteration Status. Blank Team / Project backlog is the confirmed Project-level scope.
- Permission-specific restrictions are deferred to the permissions phase.
...
| Rule | Requirement |
|---|---|
| Context source | Top workspace selector is the source of selected Project and optional Team. |
| Data visibility | Backlog, Iterations and related records are filtered to the selected Project and matching optional Team scope. |
| Create defaults | Create Work Item and Create Iteration default Project from the selected context; Team can remain blank for Project backlog scope. |
| Admin override | Workspace Admin can change Project/Team in forms where enabled. |
| Validation | Selected Team must belong to selected Project when supplied; Iteration assignment must match Work Item Project/Team scope. |
...
- Inline edit Title, Defect Priority, Plan Estimate, Owner, Schedule State, Flow State, Release.
...
| Inline edit | Schedule State and Flow State | Done | `BacklogPage.tsx` | Schedule State uses six boxes; Flow State uses dropdown; both mirror the same confirmed Work Item status value |
...
| Context default | Project auto-fill and optional Team from workspace selector | Business rule | `layout.tsx`, `BacklogPage.tsx` | Blank Team means Project backlog; Admin can override with valid Project/Team pair |
...
- Detail right panel: Project, optional Team, Start Date, End Date, State, Planned Velocity.
...
| Iterations list | Name, Theme, Start Date, End Date, Project, Planned Velocity, Task Estimate, State | Done | `IterationsPage.tsx` | Rally-like list style; Team is optional in create/detail |
...
| Quick create | New Iteration modal | Done | `IterationsPage.tsx` | Project, optional Team, Name, Start Date, End Date, State |
| Context default | Project auto-fill and optional Team from workspace selector | Business rule | `layout.tsx`, `IterationsPage.tsx` | Blank Team means Project backlog; Admin can override with valid Project/Team pair |
...
| Detail right | Project, optional Team, Start Date, End Date, State, Planned Velocity | Done | `IterationsPage.tsx` | Project/Team moved from context/subtitle |
...
- Work item Schedule State and Flow State values are `Idea / Defined / In-Progress / Completed / Accepted / Release`.
...
| Context filter | Iteration selector filtered by Project and optional Team context | Business rule | `layout.tsx`, `IterationStatusPage.tsx` | Only Iterations for selected Project and matching Team scope |
...
| Inline edit | Name, Schedule State, Flow State, Iteration, Plan Est, Owner | Done | `IterationStatusPage.tsx` | Mock local state |
| Detail panel | Iteration field | Done | `WorkItemDetailPage.tsx` | Right panel shows assignment field |
| Schedule State / Flow State | Idea, Defined, In-Progress, Completed, Accepted, Release | Done | `IterationStatusPage.tsx`, `model.ts` | Schedule State six boxes; Flow State dropdown; same mirrored Work Item value |
...
- Apply the approved Project Access model (3-level: Workspace Admin / Admin / Editor): WA/Admin edit; Editor edits assigned-Team Iteration Status; No Access hidden (implicit — no `project_members` row).
...
| P2-IS-DC-005A | Flow State uses the same values and mirrors Schedule State until separate behavior is defined | Decided |
...
| P2-CONTEXT-DC-002 | Create Work Item and Create Iteration auto-fill Project from workspace selector context; Team may remain blank for Project backlog scope | Decided |
| P2-CONTEXT-DC-003 | Workspace Admin may override Project/Team in enabled forms, but Team must belong to Project when supplied and Iteration assignment must match Project/Team scope | Decided |
| P2-CONTEXT-DC-004 | `All Teams` is not required for Iteration Status in Phase 0-4; blank Team is the Project-level scope | Decided |
...
The global workspace selector Project and optional Team context is a Phase 2 business rule, not only a UI preference. Development must use it to filter data and default Project on create flows; blank Team means Project backlog scope.
