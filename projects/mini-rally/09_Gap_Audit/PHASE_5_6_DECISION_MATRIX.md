...
| `P6-R-14` | FIX CODE (small) — enforce Rally's 13-month / 200-project limits | **Both structurally unreachable, and enforcing them would be a regression.** Every report query field is `projectId`, `teamId`, `iterationId`, `window`, `releaseId`, `unit`, `bucket`, `page`, `pageSize` (`reporting-request.dto.ts`) — there is **no date-range input at all**, and `projectId` is a single required UUID that the permission guard also resolves against. The 200-project cap cannot be approached; the 13-month cap has no parameter to bound. Rally's limits protect a general-purpose Lookback store queried across up to 200 projects — ours is a single-project, single-timebox daily snapshot table, so copying its "returns no data" behaviour would break a legitimately long release rather than protect anything. **Noted edge, not actioned:** a release whose own window exceeds ~13 months yields a correspondingly long daily axis. No reported problem; revisit if one appears |
